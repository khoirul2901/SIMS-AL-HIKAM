...
"""
content = content.replace("  const uniqueKelas = kelasData.map(k => k.namaKelas).sort();", use_effect)

# Update handleScanQR to use html5-qrcode
import re
    ...
    Swal.fire({
      ...
      didOpen: () => {
        ...
        function startScanner() {
          ...
          // attach scanner to popup for cleanup
      ...
      if (result.isConfirmed && result.value) {
        ...
        if (student) {
          ...
          Swal.fire('Gagal', 'Siswa dengan NIS tersebut tidak ditemukan atau tidak berada di kelas ini.', 'error');
        }
      }
    });
  };
...
content = re.sub(r'  const handleScanQR = \(\) => \{.*?(?=\n  return \()/s', new_scan_qr, content, flags=re.DOTALL)
