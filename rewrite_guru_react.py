...
react_code = """import React, { useState, useRef } from 'react';
...
export const MasterGuru = () => {
  ...
  const [selectedGuru, setSelectedGuru] = useState<typeof INITIAL_DATA[0] | null>(null);
  ...
  const uniqueStatusPegawai = Array.from(new Set(data.map(s => s.statusPegawai))).sort();
